import { memo, useCallback, useState, useEffect, useRef, type ElementType } from 'react'
...
const AnnouncementTabs = memo(function AnnouncementTabs({ active, onChange }: AnnouncementTabsProps) {
  ...
  return (
    <nav
      ...
      className="relative flex w-full items-stretch overflow-x-auto h-12.5 bg-[var(--ys-canvas)] shadow-[0_2px_10px_rgba(0,0,0,0.05)] scrollbar-none [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden shrink-0"
    >
      {canScrollLeft && (
        <button
          type="button"
          ...
          className="sticky left-0 flex h-full items-center justify-center bg-linear-to-r from-[var(--ys-canvas)] via-[var(--ys-canvas)] to-transparent pr-6 pl-1 @md:hidden z-10 text-[var(--ys-body-mid)] hover:text-[var(--ys-ink)]"
        >
          <ChevronLeft size={18} />
        </button>
      )}
        ...
        return (
          <button
            ...
            className={[
              ...
              'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--ys-accent-orange)] focus-visible:ring-inset',
              'motion-reduce:transition-none motion-reduce:transform-none',
              idx !== 0 ? 'border-l border-solid border-[var(--ys-mute)]' : '',
              !isActive ? 'hover:bg-[var(--ys-surface-alt)]' : '',
            ].join(' ')}
          >
            <Icon
              size={18}
              ...
              className={`transition-colors duration-200 motion-reduce:transition-none ${
                isActive ? 'text-[var(--ys-accent-orange)]' : 'text-[var(--ys-body-accent)]'
              }`}
            />
            ...
            <span
              className={`text-center text-[0.55rem] @md:text-xs leading-[1.1] whitespace-normal wrap-break-word transition-all duration-200 motion-reduce:transition-none ${
                isActive ? 'font-bold text-[var(--ys-ink)]' : 'font-medium text-[var(--ys-body)]'
              }`}
            >
            ...
            {isActive && (
              <div
                aria-hidden="true"
                className="absolute bottom-0.75 left-1/2 h-0.75 w-3/4 -translate-x-1/2 rounded-lg bg-linear-to-r from-[var(--ys-accent-orange)] to-[var(--ys-accent-light)]"
              />
            )}
          </button>
        )
      ...
      {canScrollRight && (
        <button
          ...
          className="sticky right-0 flex h-full items-center justify-center bg-linear-to-l from-[var(--ys-canvas)] via-[var(--ys-canvas)] to-transparent pl-6 pr-1 @md:hidden z-10 text-[var(--ys-body-mid)] hover:text-[var(--ys-ink)]"
        >
